...
class LagRegressionForecaster:
    ...
    def fit(self, values: list[float] | np.ndarray) -> "LagRegressionForecaster":
        ...
        features = np.vstack([data[index - self.lags : index] for index in range(self.lags, data.size)])
        targets = data[self.lags :]
        self._model = Ridge(alpha=self.alpha).fit(features, targets)
